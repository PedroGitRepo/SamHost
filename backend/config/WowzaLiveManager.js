...
class WowzaLiveManager {
    ...
    async initializeFromDatabase(userId) {
        try {
            ...
            if (serverRows.length === 0) {
                ...
                const port = server.porta_api || 6980;
    ...
    async gerenciarLiveWowza(servidor, senha, login, live, acao) {
        try {
            // Buscar configurações da API do Wowza do servidor
            const [serverRows] = await db.execute(
                'SELECT porta_api, usuario_api, senha_api FROM wowza_servers WHERE (ip = ? OR dominio = ?) AND status = "ativo" LIMIT 1',
                [servidor, servidor]
            );

            let wowzaPort = 6980;
            let wowzaUser = 'admin';
            let wowzaPassword = senha;

            if (serverRows.length > 0) {
                const serverConfig = serverRows[0];
                wowzaPort = serverConfig.porta_api || 6980;
                wowzaUser = serverConfig.usuario_api || 'admin';
            ...
            const response = await fetch(url, {
                method: 'PUT',
                headers: {
                    'Authorization': `Basic ${Buffer.from(`${wowzaUser}:${wowzaPassword}`).toString('base64')}`,
    ...
    async iniciarLive(userId, liveData) {
        try {
            ...
            const dataFimMySQL = data_fim ?
                data_fim.replace(/(\d{2})\/(\d{2})\/(\d{4})\s(.*)/, '$3-$2-$1 $4') + ':00' :
