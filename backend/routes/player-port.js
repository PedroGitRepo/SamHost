...
// GET /api/player-port/url - Obter URL base do player
router.get('/url', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      player_url: baseUrl,
      iframe_base: `${baseUrl}/api/player-port/iframe`
    });
  } catch (error) {
    console.error('Erro ao obter URL do player:', error);
    res.status(500).json({
      success: false,
      error: 'Erro ao obter URL do player'
    });
  }
});
