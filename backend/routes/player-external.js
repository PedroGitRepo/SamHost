...
router.get('/url', authMiddleware, async (req, res) => {
  try {
    ...
    let externalUrl = `https://playerv.novo.samcast.com.br/?login=${userLogin}&player=${player}`;
  ...
  try {
    ...
    if (transmissionRows.length > 0) {
      ...
      streamStatus = {
        ...
        stream_url: `http://stmv20.samcast.com.br/samhost/smil:playlists_agendamentos.smil/playlist.m3u8`,
      ...
      try {
        ...
        if (incomingStreamsResult.hasActiveStreams) {
          ...
          streamStatus = {
            ...
            stream_url: `http://stmv20.samcast.com.br/samhost/${userLogin}_live/playlist.m3u8`,
...
router.get('/embed-code', authMiddleware, async (req, res) => {
  try {
    ...
    const baseUrl = `https://playerv.novo.samcast.com.br/?login=${userLogin}&player=${player}&contador=true&compartilhamento=true`;
