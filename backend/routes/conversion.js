...
router.post('/convert', authMiddleware, async (req, res) => {
  try {
    ...
    if (outputExists.exists && !overwrite) {
      // Verificar se o arquivo convertido já está registrado no banco
      const [existingConversion] = await db.execute(
        'SELECT id FROM videos WHERE caminho = ? AND codigo_cliente = ?',
        [outputPath, userId]
      );

      if (existingConversion.length > 0) {
        // Arquivo já existe e está registrado - retornar sucesso
        return res.json({
          success: true,
          message: 'Vídeo já foi convertido anteriormente',
          conversion_id: existingConversion[0].id,
          target_bitrate: targetBitrate,
          target_resolution: targetResolution,
          quality_label: qualityLabel,
          output_path: outputPath,
          new_video_id: existingConversion[0].id,
          status: 'already_converted'
        });
      }

      // Arquivo existe no servidor mas não está registrado
      return res.status(409).json({
        success: false,
        error: 'Já existe uma conversão com essas configurações',
        requires_confirmation: true,
        existing_file: outputPath
      });
    }
    ...
    try {
      ...
      // Se delete_original estiver ativo, marcar vídeo original para remoção após conversão
      if (delete_original) {
        console.log(`🗑️ Vídeo original será removido após conversão: ${inputPath}`);
        await db.execute(
          'UPDATE videos SET compativel = "sera_removido" WHERE id = ?',
          [video_id]
        );
      }
