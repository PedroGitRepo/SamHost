...
const StreamingPlayerManager: React.FC<StreamingPlayerManagerProps> = ({
      ...
      if (response.ok) {
        ...
        if (data.is_live) {
          if (data.stream_type === 'playlist' && data.transmission) {
            ...
            const smilUrl = `http://stmv20.samcast.com.br/${userLogin}/${userLogin}/playlist.m3u8`;
            ...
            // Stream OBS
            const obsUrl = `http://stmv20.samcast.com.br/${userLogin}/${userLogin}_live/playlist.m3u8`;
          ...
          const defaultUrl = `http://stmv20.samcast.com.br/${userLogin}/${userLogin}_live/playlist.m3u8`;
  ...
  const renderPlayer = () => {
    ...
    const streamUrl = currentStreamUrl || `http://stmv20.samcast.com.br/${userLogin}/${userLogin}_live/playlist.m3u8`;
    ...
    return (
      <ClapprStreamingPlayer
        ...
        autoplay={streamStatus?.is_live || false}
        controls={true}
        ...
        onError={(error: any) => {
          console.warn('Erro no Clappr player (esperado se não houver transmissão):', error);
        }}
      />
    );
  };
