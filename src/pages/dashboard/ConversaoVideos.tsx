...
const ConversaoVideos: React.FC = () => {
  ...
  const startConversion = async (forceOverwrite: boolean = false) => {
    if (!selectedVideo) return;

    const videoId = selectedVideo.id;
    const shouldOverwrite = forceOverwrite || conversionSettings.overwrite;
    ...
    try {
      ...
      const requestBody: {
        ...
        delete_original: conversionSettings.delete_original,
        overwrite: shouldOverwrite
      };

      if (conversionSettings.use_custom) {
        requestBody.custom_bitrate = conversionSettings.custom_bitrate;
        requestBody.custom_resolution = conversionSettings.custom_resolution;
      }
      ...
      // Se requer confirmação, perguntar ao usuário
      if (result.requires_confirmation && !shouldOverwrite) {
        setConverting(prev => ({ ...prev, [videoId]: false }));

        const userConfirmed = window.confirm('Já existe uma conversão com essas configurações. Deseja sobrescrever o arquivo existente?');
      ...
      if (result.success) {
        ...
        // Resetar overwrite para futuras conversões
        setConversionSettings(prev => ({ ...prev, overwrite: false }));
      ...
      let errorMessage = 'Erro ao iniciar conversão';
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          {videos.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[...new Map(videos.map(video => [video.id, video])).values()].map((video) => (
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <div className="space-y-3">
                  <label className="flex items-center space-x-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={conversionSettings.delete_original}
                      onChange={(e) => setConversionSettings(prev => ({
                        ...prev,
                        delete_original: e.target.checked
                      }))}
                      className="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                    />
                    <div>
                      <span className="text-sm font-medium text-gray-900">Excluir vídeo original após conversão</span>
                      <p className="text-xs text-gray-500">O arquivo original será removido após a conversão bem-sucedida</p>
                    </div>
                  </label>
                </div>
              </div>
