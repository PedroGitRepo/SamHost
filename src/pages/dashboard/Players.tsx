...
const Players: React.FC = () => {
 ...
 const getActiveStreamUrl = () => {
  if (playlistTransmissionActive) {
    ...
    return `http://stmv20.samcast.com.br/${userLogin}/${userLogin}/playlist.m3u8`;
  } else if (obsStreamActive) {
    return `http://stmv20.samcast.com.br/${userLogin}/${userLogin}_live/playlist.m3u8`;
...
const getVideoUrl = (url: string, userLogin: string) => {
  if (!url) return '';
...
<script>
  ...
  const player = new UniversalPlayer({
    ...
    },
    {
      ...
      description: 'Player via subdomínio externo (playerv.novo.samcast.com.br)',
      icon: Globe,
      type: 'iframe',
      features: ['Subdomínio Dedicado', 'Isolado', 'Múltiplos Players', 'Detecção Automática'],
      code: `<iframe 
  src="https://playerv.novo.samcast.com.br/?login=${userLogin}&player=1" 
  width="640" 
  height="360" 
  frameborder="0" 
  allowfullscreen>
</iframe>`,
      previewUrl: `https://playerv.novo.samcast.com.br/?login=${userLogin}&player=1`,
      isActive: false
    },
    {
      id: 'html5',
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <ul className="text-green-800 text-sm space-y-1">
              ...
              <li>&nbsp;&nbsp;- Produção: http://novo.samcast.com.br:3001/api/player-port/iframe</li>
            </ul>
          </div>
        </div>
      </div>
