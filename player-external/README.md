# Sistema de Player Externo - playerv.novo.samcast.com.br
...
#### VOD Específico
```
https://playerv.novo.samcast.com.br/?login=usuario&vod=pasta/video.mp4&player=1
```

#### Player Mobile Otimizado
```
https://playerv.novo.samcast.com.br/?login=usuario&player=2&aspectratio=16:9&autoplay=true&muted=true
```
...
### URLs de Streaming
- **Playlist:** `http://novo.samcast.com.br:1935/samhost/{usuario}_playlist/playlist.m3u8`
- **OBS:** `http://novo.samcast.com.br:1935/samhost/{usuario}_live/playlist.m3u8`
- **VOD:** `http://novo.samcast.com.br:1935/vod/_definst_/mp4:{usuario}/{pasta}/{arquivo}/playlist.m3u8`
...
Criar entrada DNS para `playerv.novo.samcast.com.br` apontando para o servidor.
...
const generatePlayerCode = () => {
  const playerUrl = `https://playerv.novo.samcast.com.br/?login=${userLogin}&player=${selectedPlayer}&contador=true&compartilhamento=true`;
...
router.get('/external-url', authMiddleware, async (req, res) => {
  ...
  let url = `https://playerv.novo.samcast.com.br/?login=${userLogin}&player=${player}`;
